...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-cream/95 backdrop-blur border-b-2.5 border-ink">
      <div className="bg-ink text-cream text-xs"><div className="max-w-7xl mx-auto px-4 py-1.5 flex items-center justify-between gap-2"><span className="hidden sm:flex items-center gap-1.5"><MapPin size={12} /> {BRAND.addressShort}</span><span className="flex items-center gap-1.5 font-semibold"><span className="hidden sm:inline">Freshly baked every day</span><span className="sm:hidden">Fresh daily</span></span><a href={`tel:${BRAND.phone}`} className="flex items-center gap-1.5 hover:text-mustard"><Phone size={12} /> {BRAND.phone}</a></div></div>
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
        <button onClick={() => go({ name: "home" })} className="flex items-center gap-2.5 shrink-0"><Logo /><div className="text-left leading-none"><div className="font-display font-black text-xl md:text-2xl text-ink">KCB</div><div className="text-[10px] md:text-xs font-semibold text-terracotta tracking-widest uppercase">KiNGS Cakes Bakes</div></div></button>
...
function Logo() { return <img src="/logo.png" alt="KCB KiNGS Cakes Bakes logo" className="w-11 h-11 md:w-12 md:h-12 rounded-full border-2.5 border-ink nb-shadow-sm shrink-0 object-cover" />; }
