...
interface AppState {
  ...
  setCartWeight: (productId: string, oldWeight: string, item: CartItemT) => void;
...
export const useApp = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      updateCartQty: (productId, weight, qty) =>
        set((s) => ({
          cart: s.cart
            .map((c) =>
      ...
      setCartWeight: (productId, oldWeight, item) =>
        set((s) => {
          const without = s.cart.filter(
            (c) => !(c.productId === productId && c.weight === oldWeight)
          );
      ...
      removeFromCart: (productId, weight) =>
        set((s) => ({
          cart: s.cart.filter(
            (c) => !(c.productId === productId && c.weight === weight)
          ),
        })),
      clearCart: () => set({ cart: [] }),
      cartCount: () => get().cart.reduce((n, c) => n + c.qty, 0),
      cartSubtotal: () =>
        get().cart.reduce((n, c) => n + c.qty * c.price, 0),
