...
export async function bakeryContext(): Promise<string> {
  const products = await db.product.findMany({ where: { status: "active" }, include: { weights: true, category: true } });
  const productList = products.map((p) => { const prices = p.weights.map((w) => `₹${w.price}/${w.weight}lb`).join(", "); return `- ${p.name} (${p.category.name}) — ${p.eggless ? "eggless, " : ""}${prices}; prep ${p.prepHours}h`; }).join("\n");
...
- Store pickup and home delivery orders placed online
- Delivery address, delivery date/time, pickup date/time, and order fulfilment
...
- Fulfilment: store pickup and home delivery
- Delivery fee: ${BRAND.deliveryFee === 0 ? "currently free" : `₹${BRAND.deliveryFee}`}
...
- For delivery questions, explain that customers can choose Home Delivery at checkout and must provide a complete delivery address.`;
}
