  ...
  return (
    <div className="min-h-screen flex flex-col bg-background doodle-bg">
      ...
      <main className="flex-1 w-full">
        ...
        {view.name === "search" && <SearchView query={view.query} category={view.category} />}
        {view.name === "product" && <ProductView key={view.slug} slug={view.slug} />}
        {view.name === "cart" && <AuthGuard feature="view your cart"><CartView /></AuthGuard>}
        {view.name === "checkout" && <AuthGuard feature="checkout and place an order"><CheckoutView /></AuthGuard>}
        {view.name === "confirmation" && <AuthGuard feature="view your order confirmation"><ConfirmationView orderId={view.orderId} /></AuthGuard>}
