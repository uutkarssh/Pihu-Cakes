...
export function CartView() {
  ...
  const setCartWeight = useApp((s) => s.setCartWeight);
  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 md:py-24 text-center">
        ...
        <BakeryButton className="mt-6" onClick={() => navigate({ name: "search" })}>
          Browse all cakes
        </BakeryButton>
      </div>
    );
          ...
          {cart.map((item) => (
            <BakeryCard key={`${item.productId}-${item.weight}`} className="p-4 flex gap-4">
              <button
                ...
                className="w-24 h-24 rounded-xl border-2 border-ink overflow-hidden shrink-0 nb-shadow-sm"
              >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-end justify-between mt-3 gap-3">
                  <div className="flex items-center gap-2">
                    <div className="flex items-center bg-cream border-2 border-ink rounded-full nb-shadow-sm">
                      <button
                        onClick={() => updateQty(item.productId, item.weight, item.qty - 1)}
                        className="w-8 h-8 flex items-center justify-center hover:bg-mustard/30 rounded-l-full"
                      >
                        <Minus size={14} />
                      </button>
                      ...
                      <button
                        onClick={() => updateQty(item.productId, item.weight, item.qty + 1)}
                        className="w-8 h-8 flex items-center justify-center hover:bg-mustard/30 rounded-r-full"
                      >
                        <Plus size={14} />
                      </button>
                    </div>
        ...
        <div className="lg:col-span-1">
          <BakeryCard className="p-5 sticky top-28">
            ...
            <div className="mb-4">
              ...
              {applied ? (
                <div className="flex items-center justify-between bg-mustard/30 border-2 border-ink rounded-xl px-3 py-2">
                  <span className="flex items-center gap-1.5 font-bold text-sm">
                    <CheckCircle2 size={15} className="text-terracotta" /> {applied.code}
                  </span>
                  <button
                    onClick={() => {
                      setApplied(null);
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-cream border-2 border-ink rounded-xl px-3 py-2 text-sm outline-none uppercase"
                  />
                  <BakeryButton variant="cream" className="!px-4 !py-2 text-sm" onClick={applyCoupon} disabled={checking}>
                    {checking ? "…" : "Apply"}
                  </BakeryButton>
                </div>
              )}
              <div className="text-[11px] text-muted-foreground mt-1.5">
                Try: FRESH10 · PIHU100 · WELCOME150
              </div>
            </div>
            ...
            <div className="text-[11px] text-muted-foreground text-center mt-3 flex items-center justify-center gap-1">
              <ShoppingBag size={12} /> Pickup only · No home delivery
            </div>
          </BakeryCard>
        </div>
      </div>
